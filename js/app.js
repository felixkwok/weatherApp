...
//Declare HK observatory variables
var Weekarray =[];
var Maxarray =[];
var Minarray =[];
...
//init method
function initialize() {
    document.addEventListener("deviceready", TestGeo, false);

    //check if phone GPS enabled
    cordova.plugins.diagnostic.isGpsLocationEnabled(enabled(e),positionError(error), {enableHighAccuracy: true});
    var watchID = navigator.geolocation.watchPosition(getLocation, positionError, { timeout: 30000, enableHighAccuracy: true});
...
function TestGeo(){
    ...
    navigator.geolocation.getCurrentPosition(getLocation, positionError, {enableHighAccuracy: true}); 
}
